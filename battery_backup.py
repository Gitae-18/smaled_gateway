...
FORCE_BACKUP_DONE_ON_START = False
ALLOW_SHUTDOWN = False
HOLD_OFF_S = 2.0

BACKUP_SCRIPT = "/home/pi/backup.py"
...
def is_backup_done() -> bool:
    if FORCE_BACKUP_DONE_ON_START:
        return True
    return os.path.exists(BACKUP_FLAG_FILE)
...
def cut_battery_poweroff():
    print("[ACTION] Backup Complete. System Off", flush=True)
    if ALLOW_SHUTDOWN:
...
def main():
    ...
    print(f"[INFO] 백업 플래그 파일: {BACKUP_FLAG_FILE}", flush=True)
    print(
        f"[INFO] ALLOW_SHUTDOWN={ALLOW_SHUTDOWN}, FORCE_BACKUP_DONE_ON_START={FORCE_BACKUP_DONE_ON_START}",
        flush=True,
    )
    ...
    try:
        while True:
            ...
            if not current_adapter:
                ...
                if (not is_adapter_on()) and power_loss_detected and is_backup_done():
